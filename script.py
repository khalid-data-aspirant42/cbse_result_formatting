def get_result():
    ...
    for file_name in os.listdir(dir_path):
        ...
        if os.path.isfile(os.path.join(dir_path, file_name)):
            # Check if the file meets some criteria (e.g. has a certain extension)
    ...
    for files in file_list:
        ...
        import os
        if bool(re.search(r"\d+",my_new_lst3[2][5][0]))==False:
            filename = "output/class_12.xlsx"
            counter = 1
            while os.path.exists(filename):
                filename = f"output/class_12_{counter}.xlsx"
                counter += 1
            cbse_result.to_excel(filename, index=False)
        else:
            filename = "output/class_10.xlsx"
            counter = 1
            while os.path.exists(filename):
                filename = f"output/class_10_{counter}.xlsx"
                counter += 1
            cbse_result.to_excel(filename, index=False)
    # --------end
    return ''
